...
## 추가 수정 2: React Hydration Error #418

### 문제
시크릿모드에서도 `React error #418` (Hydration 에러) 발생
...
```
`Math.random()`이 `useState` 초기값에서 호출되어 서버 렌더링과 클라이언트 렌더링 결과가 불일치

### 해결
```typescript
// Good: 초기값 고정, 클라이언트에서만 랜덤 선택
const [loadingMessage, setLoadingMessage] = useState(LOADING_MESSAGES[0])
useEffect(() => setLoadingMessage(getRandomLoadingMessage()), [])
```

### 수정 파일
- [page.tsx](file:///C:/Users/Administrator/Desktop/채용공고/web/app/jobs/page.tsx) (라인 471-476)

---

## 오늘 커밋 히스토리

1. `99a3bcf` - feat: 직무 동의어 매핑으로 필터링 개선
2. `9a80eb6` - fix: 새로고침 시 필터 초기화 버그 수정 (options null 체크)
3. `e11bf82` - fix: user_profiles/user_preferences 조회 시 maybeSingle 사용
4. `f9fde64` - fix: React Hydration Error #418 수정
